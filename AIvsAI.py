...
def run():
    params1 = './params/AlphaZero_NewEval_current.pt'
    params2 = './params/AlphaZero_NewEval_current.pt'
    try:
        env = Env()
        game = Game(env)
        net1 = PolicyValueNet(0, params1, device)
        az1 = MCTSPlayer(net1.policy_value_fn, c_puct=2, n_playout=1000, is_selfplay=0)
        az1.eval()
        net2 = PolicyValueNet(0, params2, device)
        az2 = MCTSPlayer(net2.policy_value_fn, c_puct=2, n_playout=1000, is_selfplay=0)
        az2.eval()
